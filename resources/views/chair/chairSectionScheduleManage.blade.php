...
<body>
    ...
    <div class="scrollable-form">
        ...
        <div class="container-fluid p-5 mb-5 rounded" style="margin-top: 120px;">
                    ...
                    <table class="text-center shadow-lg table-sched" id="scheduleTable">
                        ...
                        <tbody>
                            ...
                            @foreach ($timeSlots as $timeSlot)
                                <tr class="border">
                                    <td class="border text">{{ $timeSlot['start'] }}<br>{{ $timeSlot['end'] }}</td>

                                    @foreach ($daysOfWeek as $day)
                                        <td class="border text">
                                            @foreach ($section_schedules as $schedule)
                                                @if ($schedule->day == $day && $schedule->start_time < $timeSlot['end'] && $schedule->end_time > $timeSlot['start'])
        ...
        @if ($currentSchoolYear == $sectionSchoolyear)

            <div style="width:1800px; margin-top: 15px; margin: 0px auto;">
                <hr>
                <div class="container-fluid mt-5 mb-5 edit-user-container">
                    <div class="container-fluid">
                            ...
                            <tbody class="text1">
                                @foreach ($subjects as $subject)
                                    <tr>
                                        ...
                                        <td>{{ $subject->cs_yearLevel }}</td>
                                        <td>{{ $subject->cs_semesterOffered }}</td>
                                        <td>
                                                ...
                                                @for ($hour = 8; $hour <= 17; $hour++)
                                                    @for ($min = 0; $min < 60; $min += 30)
                                                        @php
                                                            ...
                                                            $displayTime = ($hour > 12 ? $hour - 12 : $hour) . ':' . sprintf('%02d', $min) . ($hour >= 12 ? ' PM' : ' AM');
                                                        @endphp
                                                        <option value="{{ $time }}">{{ $displayTime }}
                                                        </option>
                                                    @endfor
                                                @endfor
                                            </select>
                                            ...
                                            <select class="form-control" name="facultyId">
                                                ...
                                                @foreach ($faculties as $faculty)
                                                    <option value="{{ $faculty->faculty_id }}">
                                                        {{ $faculty->faculty_firstName . ' ' . $faculty->faculty_lastName . '->' . $faculty->dept_desc }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </td>
                                            ...
                                            <select class="form-control" name="roomId">
                                                ...
                                                @foreach ($rooms as $room)
                                                    <option value="{{ $room->room_code }}">
                                                        {{ $room->room_desc . '->' . $room->room_location }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
                                            <button style="height:50px;"
                                                class="btn fw-bold btn-success addToSchedule">Add Schedule</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        ...
                        <form action="" method="POST">
                            ...
                            <div class="form-group rounded">
                                ...
                                <button type="submit" style="height:40px;"
                                    class="ex btn  w-100 fw-bold p-3 btn-primary">LOAD</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div class="container">
            ...
            <div class="col mb-5 mt-5">
                <table id="user_sched" class="table table-striped" style="width:100%">
                    <thead>
                        <tr>
                            <th>Subject</th>
                            <th>Room</th>
                    ...
                    <tbody>
                        @foreach ($section_schedules as $section_schedule)
                            <tr class="fw-bold">
                                ...
                                <td>{{ $section_schedule->faculty_firstName }}
                                    {{ $section_schedule->faculty_lastName }}</td>
                                ...
                                <td>
                                    <form
                                        action="{{ route('deleteSchedule', $section_schedule->sectionSchedule_id) }}"
                                        ...
                                        <div class="text-center">
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="btn w-100 fw-bold p-3 delete-button btn-danger"><i
        ...
        $(document).ready(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $.each(data.filteredSubjects, function(index, subject) {
                                ...
                                row += "<td class=''>" + subject.cs_semesterOffered +
                                    "</td>";
                                ...
                                for (let hour = 8; hour <= 17; hour++) {
                                    for (let min = 0; min < 60; min += 30) {
                                        let time = ('0' + hour).slice(-2) + ':' + ('0' +
                                            min).slice(-2);
                                        let displayTime = (hour > 12 ? hour - 12 :
                                                hour) +
                                ...
                                // Room
                                row +=
                                    '<td><select class="form-control" name="roomId">';
            ...
            $(document).on('click', '.addToSchedule', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.sectionCapacityExceedsRoomCapacity) {
                            alert("Section capacity is greater than room capacity");
                            // Handle what you want to do when there's a capacity issue
                        } else {
                            if (data.conflictingSchedules && data.conflictingSchedules.length >
                                0) {
            ...
            $.ajax({
                ...
                success: function(scheduleData) {
                    ...
                    scheduleData.forEach(function(schedule) {
                        ...
                        $("#scheduleTable td[data-day='" + day + "'][data-time='" + time + "']")
                            .text(schedule.subject_name);
                    });
                },
