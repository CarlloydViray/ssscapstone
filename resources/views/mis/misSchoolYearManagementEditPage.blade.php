...
<a href="/misSchoolyearManagementResource" class="fw-bold link p-5" ><i class="fa-solid fa-arrow-left"></i> Back</a>
...
<div class="container  shadow-lg w-50  mb-5 rounded" style="width:500px; margin-top:170px;">
            <div class="container-fluid shadow-lg edit-user-container">
                <div class="container-fluid">
                    <h1 class="edit-user-title">Edit Scholl Year</h1>
                </div>
            </div>
        <div class="container" style="margin-left:150px;">

        @foreach ($schoolyears as $schoolyear)
            <form action="{{ $schoolyear->schoolyear_id }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                <div class="col-md-8 col-sm-12 col-12 p-3">
                    ...
                    <select name="campus_code" id="campusName" class="form-control">
                        ...
                        @foreach ($campuses->where('campus_code', $schoolyear->campus_code) as $campus)
                            <option value="{{ $campus->campus_code }}"
                                {{ $schoolyear->campus_code == $campus->campus_code ? 'selected' : '' }}>
                ...
                <div class="row">
                <div class="col-md-8 col-sm-12 col-12 p-3">
                    <label for="schoolyearSy" class="form-label">School Year</label>
                    <select name="schoolyear_sy" id="schoolyearSy" class="form-control">
                        @php
                        ...
                        @for ($year = $currentYear; $year <= $currentYear + $futureYears; $year++)
                            ...
                            @endphp
                            <option value="{{ $academicYear }}" @if ($schoolyear->schoolyear_sy == $year) selected @endif>
                                {{ $academicYear }}</option>
                        @endfor
                    </select>

                    <small class="form-text text-muted">Please select the school year from the list.</small>
                    @error('schoolyear_sy')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="col-md-6 col-sm-12 col-12">
                    <input type="submit"  class="btn w-100  mb-4 p-3 fw-bold mt-3 btn-primary btn-block" value="Update">
                    </div>
                </center>
