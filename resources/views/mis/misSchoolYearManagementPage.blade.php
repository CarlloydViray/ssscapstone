...
<head>
    ...
    <title>Schoolyear Management</title>
</head>
<script>
...
<body>
    ...
    <div id="mySidebar" class="sidebar shadow-lg">
        ...
        <a href="misFacultyManagement" class=" fw-bold"><i class="fa-solid fa-users"></i>&nbsp;&nbsp;Faculty
            Management</a>
        <a href="misSchoolyearManagement" class="active fw-bold"><i
                class="fa-solid fa-calendar-days"></i>&nbsp;&nbsp;School
            Year Management</a>
    ...
    <div id="main" class="content scrollable-form">
        ...
        <div class="navbar shadow-lg rounded">
            <div class="navbar-title">
                <h1 class="title fw-bold p-3"> SCHOOLYEAR MANAGEMENT</h3>
            </div>
        ...
        <div class="container-fluid shadow rounded mt-5 p-5 ">
            ...
            <a class="btn btn-danger btn-lg ms-3  p-3 fw-bold float-end" href="{{ route('schoolyear.export') }}"><i
            ...
            <button type="button" class="btn p-3 float-end ms-3 btn-lg fw-bold btn-primary" data-bs-toggle="modal"
                data-bs-target="#import"><i class="fa-solid fa-lg fa-file-import"> </i> &nbsp; Import Campus
                Data</button>
                        ...
                        <tbody>
                            @foreach ($schoolyears as $schoolyear)
                                <tr class="fw-bold" style="font-size: 10px;">
                                    ...
                                    <td>{{ $schoolyear->campus_code }}</td>
                                    <td>{{ $schoolyear->schoolyear_sy }}</td>
                                    <td>{{ date('M d, Y h:i A', strtotime($schoolyear->created_at)) }}</td>
                                    <td>{{ date('M d, Y h:i A', strtotime($schoolyear->updated_at)) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    ...
                    <div class="col-md-2 rounded shadow-lg p-4">
                        <form action="" method="POST">
                            ...
                            <div class="form-group rounded">
                                <label class="text1 fe-bold mb-1" for="curriculum">Campus:</label>
                        ...
                        <br>
                        <button type="button" id="showAllSchoolyearsBtn"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="userModalLabel">ADD NEW SCHOOL YEAR</h5>
                    <button type="button" class="btn-close btn-lg" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <!-- Your form content goes here -->
                    <form action="{{ route('misSchoolyearManagementResource.store') }}" method="POST"
                        ...
                        <div class="row forms">
                            ...
                            @endif
                            <div class="col-md-12 col-sm-12 col-12 p-3">
                                <label for="schoolyearSy" class="form-label">School Year <span
                                        style="color: #fc0905;"> *</span></label>
                                <select name="schoolyear_sy" id="schoolyearSy" class="form-control" required>
                                    @php
                                        $currentYear = now()->year;
                                        $selectedYear = old('schoolyear_sy') !== null ? (int) old('schoolyear_sy') : $currentYear;
                                ...
                                <small class="form-text text-muted">Please select the school year from the
                                    list.</small>
                                @error('schoolyear_sy', 'schoolyearValidation')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
    ...
    <div class="modal fade" id="import" tabindex="-1" aria-labelledby="userModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold"id="userModalLabel">IMPORT SCHOOLYEAR DATA</h5>
                    <button type="button" class="btn-close btn-lg" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <!-- Your form content goes here -->
                    <form action="{{ route('schoolyear.import') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row forms">
                            <div class="col-md-12 col-sm-12 col-12 p-3">
        ...
        $(document).ready(function() {
            $("form").submit(function(e) {
                if ($(this).find('.ex').length > 0) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('filterCampusSchoolyearRoute') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            campus: campus,
                        ...
                        success: function(data) {
                            $("#users tbody").empty();
                            console.log(data.filteredSchoolyears);

                            if (data.filteredSchoolyears.length > 0) {
                                $.each(data.filteredSchoolyears, function(index, schoolyear) {
                                    var createdDate = new Date(schoolyear.created_at);
                                    var updatedDate = new Date(schoolyear.updated_at);
            ...
            $("#showAllSchoolyearsBtn").click(function() {
                $.ajax({
                    type: "POST",
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            $.each(data, function(index, schoolyear) {
                                var createdDate = new Date(schoolyear.created_at);
                                var updatedDate = new Date(schoolyear.updated_at);
