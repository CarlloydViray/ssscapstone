...
<body>
    ...
    <div id="mySidebar" class="sidebar shadow-lg">
        ...
        <a href="misRoomManagement" class="active fw-bold"><i class="fa-solid fa-house"></i>&nbsp;&nbsp;Rooms
            Management</a>
    ...
    <div id="main" class="content scrollable-form">
        ...
        <div class="navbar shadow-lg rounded">
            <div class="navbar-title">
                <h1 class="title fw-bold p-3"> ROOMS MANAGEMENT</h3>
            </div>
        ...
        <div class="container-fluid shadow rounded mt-5 p-5 ">
            ...
            <a class="btn btn-danger btn-lg ms-3  p-3 fw-bold float-end"href="{{ route('room.export') }}"><i
                    class="fa-solid fa-lg  fa-file-export"></i> &nbsp; Export Rooms Data</a>
            <button type="button" class="btn p-3 float-end ms-3 btn-lg fw-bold btn-primary" data-bs-toggle="modal"
                data-bs-target="#import"><i class="fa-solid fa-lg fa-file-import"> </i> &nbsp; Import Rooms
                Data</button>
            ...
            <div class="row mt-5">
                        ...
                        <tbody>
                            @foreach ($rooms as $room)
                                <tr class="fw-bold" style="font-size: 10px;">
                                    ...
                                    <td>
                                        <form
                                            action="{{ route('misRoomManagementResource.destroy', $room->room_code) }}"
                                            role="alert" method="post" alert-title="Delete Room?"
                                            ...
                                            <div class="text-center">
                                                @csrf
                                                @method('DELETE')
                                                <a class="btn btn-warning" href="/edit/room/{{ $room->room_code }}">
                ...
                @if ($userType !== 'mis')
                    <div class="col-md-2 rounded shadow-lg p-4">
                        ...
                        <br>
                        <button type="button" id="showAllRoomsBtn"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="userModalLabel">ADD NEW ROOM</h5>
                    ...
                    <form action="{{ route('misRoomManagementResource.store') }}" method="POST"
                        ...
                        <div class="row forms">
                            ...
                            <div class="col-md-6 col-sm-12 col-12 p-3">
                                <label for="roomCode" class="form-label">Room Code <span style="color: #fc0905;">
                                        *</span></label>
                                <input type="text" name="room_code" id="roomCode" class="form-control"
                                    placeholder="Input Room Code" value="{{ old('room_code') }}" required>
                                @error('room_code', 'roomValidation')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-md-6 col-sm-12 col-12 p-3">
                                <label for="roomCapacity" class="form-label">Room Capacity <span
                                        style="color: #fc0905;"> *</span></label>
                                <input type="number" name="room_capacity" MIN=0 id="roomCapacity"
                                    class="form-control" placeholder="Input Room Capacity"
                                    value="{{ old('room_capacity') }}" required>
                                @error('room_capacity', 'roomValidation')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-md-12 col-sm-12 col-12 p-3">
                                <label for="roomDesc" class="form-label">Room Description <span
                                        style="color: #fc0905;"> *</span></label>
                                <textarea name="room_desc" id="roomDesc" class="form-control" placeholder="Input Room Description" required>{{ old('room_desc') }}</textarea>
                                @error('room_desc', 'roomValidation')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>

                            <div class="col-md-12 col-sm-12 col-12 p-3">
                                <label for="roomLocation" class="form-label">Room Location <span
                                        style="color: #fc0905;"> *</span></label>
                                <textarea name="room_location" id="roomLocation" class="form-control" placeholder="Input Room Location" required>{{ old('room_location') }}</textarea>
                                @error('room_location', 'roomValidation')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
        ...
        $(document).ready(function() {
            $("form").submit(function(e) {
                if ($(this).find('.ex').length > 0) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('filterCampusRoomsRoute') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            campus: campus,
                        ...
                        success: function(data) {
                            $("#users tbody").empty();
                            console.log(data.filteredRooms);

                            if (data.filteredRooms.length > 0) {
                                $.each(data.filteredRooms, function(index, room) {
                                    var createdDate = new Date(room.created_at);
                                    var updatedDate = new Date(room.updated_at);
                                    ...
                                    var newRow =
                                        ...
                                        '<td>' +
                                        '<form action="/misRoomManagementResource/' +
                                        room.room_code + '" ' +
                                        'role="alert" method="post" alert-title="Delete Room?" ' +
                                        ...
                                        '<a class="btn btn-warning" href="/edit/room/' +
                                        room.room_code + '">' +
                                        '<i class="fa-solid fa-pen-to-square"></i> </a>' +
            ...
            $("#showAllRoomsBtn").click(function() {
                $.ajax({
                    type: "POST",
                    url: "{{ route('allRoomsRoute') }}",
                    data: {
                        _token: "{{ csrf_token() }}",
                    },
                    success: function(data) {
